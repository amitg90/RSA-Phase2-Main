...
def next_page(limit=10, start_after=None):
    ...
    query = query.order_by(u'unitprice').limit(limit)
...
def update_inventory(item_id, units):
    db = firestore.Client()
    item_ref = db.collection(u'Product').document(item_id)
    item = document_to_dict(item_ref.get())
    quantity = max(0, item.get("qoh") - units)
    if not quantity:
        return False
...
def delete(id):
    db = firestore.Client()
    item_ref = db.collection(u'Product').document(id)
    item_ref.delete()
